...
mutable struct SPSCStorage
    ...
    """
    function SPSCStorage(ptr::Ptr{T}; finalizer_fn::FFn=s -> nothing) where {T,FFn<:Function}
        ...
        storage_size::UInt64 = unsafe_load(
            reinterpret(Ptr{UInt64}, ptr8 + SPSC_STORAGE_SIZE_OFFSET)
        )
    ...
    function SPSCStorage(
        ptr::Ptr{T}, storage_size::Integer; finalizer_fn::FFn=s -> nothing
    ...
    The total size is calculated as follows: `storage_size = SPSC_BUFFER_OFFSET + buffer_size`.
    """
    function SPSCStorage(storage_size::Integer)
...
function storage_write_metadata!(
    ...
    unsafe_store!(
        reinterpret(Ptr{UInt64}, storage_ptr + SPSC_CACHE_LINE_SIZE * 1), read_ix, :release
    ) # read_ix (0-based)
    unsafe_store!(
        reinterpret(Ptr{UInt64}, storage_ptr + SPSC_CACHE_LINE_SIZE * 2), write_ix, :release
    ) # write_ix (0-based)
    unsafe_store!(
        reinterpret(Ptr{UInt64}, storage_ptr + SPSC_CACHE_LINE_SIZE * 3), msg_count, :release
    ) # msg_count
    nothing
end
