...
mutable struct SPSCQueueVar
    ...
    function SPSCQueueVar(storage::SPSCStorage)
        ...
        new(storage, calc_max_message_size(storage), buf_size)
    end
end
...
@inline function Queues.enqueue!(queue::SPSCQueueVar, msg::Message)::Bool
    ...
    if next_write_ix < queue.buffer_size
        ...
        # copy payload + header
        unsafe_copyto!(
            queue.storage.buffer_ptr + write_ix + sizeof(UInt64), msg.data, payload_size(msg)
        )
        unsafe_store!(
            reinterpret(Ptr{UInt64}, queue.storage.buffer_ptr + write_ix), UInt64(payload_size(msg))
        )
    else
        # crossing the end
        ...
        unsafe_copyto!(queue.storage.buffer_ptr + sizeof(UInt64), msg.data, payload_size(msg))
        unsafe_store!(reinterpret(Ptr{UInt64}, queue.storage.buffer_ptr), UInt64(payload_size(msg)))
