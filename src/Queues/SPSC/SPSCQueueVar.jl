...
@inline buffer_size(queue::SPSCQueueVar) = queue.storage.buffer_size
...
@inline function Queues.enqueue!(queue::SPSCQueueVar, msg::Message)::Bool
	@assert payload_size(msg) > 0 "Message payload must be > 0"
	...
	if next_write_ix < queue.storage.buffer_size
		...
		if write_ix < queue.cached_read_ix && sentinel_end ≥ queue.cached_read_ix
			queue.cached_read_ix = unsafe_load(queue.storage.read_ix, :acquire)
			if write_ix < queue.cached_read_ix && sentinel_end ≥ queue.cached_read_ix
